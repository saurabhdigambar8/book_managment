...
const getBookById= async function(req,res){
    try{
        ...
        if(!books||books.isDeleted==false){
...
const updateBook=async function(req, res){
    try{
          ...
          if(titleCheck||typeof title!="string",title.trim().length==0){
          ...
          if(ISBNCheck||typeof ISBN!="string",ISBN.trim().length==0){
          ...
          return res.status(200).send({status:true,message:"sucess",data:updatedBook})
    }catch(err){
